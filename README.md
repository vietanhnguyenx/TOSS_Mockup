...
| Performance Factor | Operations Analysis | v0.23 | [performance-factor.html](https://vietanhnguyenx.github.io/TOSS_Mockup/performance-factor.html) |
...
| **Import Performance Data** | Pick the source branch — Airbus (PEP), Boeing (PET) or ATR — load its result files, review the flight-level validation preview, then write the data in. The `Match` column flags readings whose flight is unclear; they are written and sent to the Integration Hub monitoring screen |
